...
import { doc, getDoc, setDoc, addDoc, collection, runTransaction, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
async function obtenerNumeroOS() {
    ...
    const sufijo = configDocumentos.os?.correlativoSufijo || 'D';
    return runTransaction(db, async t => {
        const ref = doc(db,'config','contadorOS');
        const snap = await t.get(ref);
        const n = snap.exists() ? Number(snap.data().ultimoNumero || 0) + 1 : 1;
        t.set(ref,{ ultimoNumero:n, updatedAt:serverTimestamp() },{ merge:true });
        return `${String(n).padStart(7,'0')}${sufijo}`;
    });
}
...
async function guardarOS() {
    ...
    try {
        ...
        console.error(e);
        alert('❌ No se pudo registrar la OS. Revisa los permisos de Firestore para ordenesServicio y config/contadorOS.');
    } finally {
        btn.disabled = false;
