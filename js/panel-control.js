...
async function cargarPanel() {
    ...
    try {
        ...
        $('panelMensaje').textContent = 'No se pudieron cargar todos los indicadores. Revisa los permisos de lectura de Firebase.';
    } finally {
...
onAuthStateChanged(auth, async user => {
    ...
    const rol = String(perfil.data()?.rol || '').toLowerCase();
    if (rol !== 'admin') return location.href='menu2.html';
    await cargarPanel();
});
