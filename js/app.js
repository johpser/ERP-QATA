...
// --- 4. PDF: se genera mediante js/pdf-order.js para que OC y OS usen el mismo formato. ---
...
function actualizarTipoDocumentoUI() {
    const tipo = document.getElementById('tipoDocumento')?.value || 'OC';
    const titulo = tipo === 'OS' ? 'ORDEN DE SERVICIO' : 'ORDEN DE COMPRA';
    const tituloEl = document.getElementById('tituloOrden');
    const numeroEl = document.getElementById('ocNumero');
    const boton = document.getElementById('btnFinalizar');
    const detalle = document.getElementById('tituloDetalle');
    if (tituloEl) tituloEl.textContent = titulo;
    if (numeroEl) numeroEl.textContent = `${tipo}-AL-GUARDAR`;
    if (detalle) detalle.textContent = tipo === 'OS' ? 'DETALLE DE SERVICIOS' : 'DETALLE DE PRODUCTOS';
    if (boton) boton.innerHTML = `<i class="bi bi-save"></i> GUARDAR ${tipo === 'OS' ? 'ORDEN DE SERVICIO' : 'ORDEN DE COMPRA'}`;
}
...
const iniciarApp = async () => {
    ...
    document.getElementById('tipoDocumento')?.addEventListener('change', actualizarTipoDocumentoUI);
...
async function guardarOrden() {
    if (productosTabla.length === 0) return alert("La tabla de productos/servicios está vacía.");
    const rucProv = document.getElementById('rucProv').value.trim();
    if (!rucProv) return alert("Ingrese el RUC del proveedor.");

    const tipoDocumento = document.getElementById('tipoDocumento')?.value === 'OS' ? 'OS' : 'OC';
    const btn = document.getElementById('btnFinalizar');
    btn.disabled = true;
    btn.innerText = "⌛ REGISTRANDO...";

    try {
        const contadorId = tipoDocumento === 'OS' ? 'contadorOS' : 'contadorOC';
        const nroDocumento = await runTransaction(db, async (t) => {
            const ref = doc(db, "config", contadorId);
            const s = await t.get(ref);
            const n = s.exists() ? Number(s.data().ultimoNumero || 0) + 1 : 1;
            t.set(ref, { ultimoNumero: n });
            return `${tipoDocumento}-${n.toString().padStart(4, '0')}`;
        });

        const data = {
            tipoDocumento,
            nroDocumento,
            nroOC: nroDocumento, // compatibilidad con registros y pantallas anteriores
            nroGR: tipoDocumento === 'OC' ? `GR-${nroDocumento.split('-')[1]}` : '',
            ...
            proveedor: {
                ...
                formaPago: document.getElementById('pagoProv').value,
        ...
        // OC y OS comparten el historial de órdenes. La guía se genera únicamente desde su módulo,
        // evitando crear guías fantasma al registrar una orden.
        ...
        alert(`✅ Registrado con éxito: ${nroDocumento}`);
        location.reload();
    } catch (e) {
        console.error(e);
        alert("❌ No se pudo registrar la orden. Revisa la consola y los permisos de Firebase.");
        btn.disabled = false;
        actualizarTipoDocumentoUI();
    }
}
