...
function renderizarTabla(snapshot) {
    ...
    snapshot.forEach((docSnap) => {
        ...
        const formaPago = d.proveedor?.pago || d.proveedor?.formaPago || '---';
        const nroDocumento = d.nroDocumento || d.nroOC || 'S/N';
        const tipoDocumento = d.tipoDocumento || (String(nroDocumento).startsWith('OS-') ? 'OS' : 'OC');
        ...
        fila.innerHTML = `
            <td class="fw-bold">${nroDocumento}</td>
            <td><span class="badge ${tipoDocumento === 'OS' ? 'text-bg-info' : 'text-bg-dark'}">${tipoDocumento}</span></td>
...
tablaHistorial.addEventListener('click', async (e) => {
    ...
    if (target.classList.contains('btn-pdf')) {
        ...
        if (docSnap.exists()) {
            ...
            // Compatibilidad con órdenes históricas emitidas antes de guardar una copia
            // de los términos/configuración dentro de cada documento.
...
function aplicarFiltros() {
    ...
    filas.forEach(fila => {
        ...
        const valorProyecto = fila.cells[8].innerText.toUpperCase();
        const valorVencimiento = fila.cells[6].innerText; // Columna Vencimiento

        const coincideBusqueda = textoFila.includes(term);
        const coincideProyecto = proyecto === "" || valorProyecto === proyecto;
        const nroDocumento = fila.cells[0]?.innerText || "";
        const tipoFila = nroDocumento.startsWith("OS-") ? "OS" : "OC";
        const coincideTipo = tipoDoc === "" || tipoFila === tipoDoc;
        ...
        fila.style.display = (coincideBusqueda && coincideProyecto && coincideTipo && coincideSolped && coincidePago && coincideFecha) ? '' : 'none';
    });
}
...
document.getElementById('btnLimpiarFiltros').addEventListener('click', () => {
    ...
    if (document.getElementById('filtroTipoDoc')) document.getElementById('filtroTipoDoc').value = "";
...
document.getElementById('filtroTipoDoc')?.addEventListener('change', aplicarFiltros);
    ...
    filas.forEach(f => {
        if (f.style.display !== 'none') {
            ...
            datosExcel.push({
                ...
                "FORMA DE PAGO": tds[2].innerText,
                "FACTURA": ins[0]?.value || '',
                "FECHA": tds[4].innerText,
                "COTIZACION": tds[5].innerText,
                "VENCIMIENTO": tds[6].innerText,
                "PROVEEDOR": tds[7].innerText,
                "PROYECTO": tds[8].innerText,
                "COD. SOLPED": ins[1]?.value || '',
                "ESTADO SOLPED": f.querySelector('[data-campo="estadoSolped"]')?.innerText.trim() || '',
                "ESTADO PAGO": f.querySelector('[data-campo="estadoPago"]')?.innerText.trim() || '',
                "TOTAL": tds[12].innerText
            });
        }
    });
...
async function cargarFiltrosConfigurables() {
    try {
        ...
        console.warn('No se pudieron cargar proyectos configurables en el filtro.', error);
    }
}
