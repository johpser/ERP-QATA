...
import { modulosPorPerfil, normalizarRol } from './access-control.js';

const tablaHistorial = document.getElementById('tablaHistorial');
let rolActual = '';
let perfilActual = {};
const soloLectura = () => !modulosPorPerfil(perfilActual).includes('oc_create');
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        try {
            ...
            if (userDoc.exists()) {
                perfilActual = userDoc.data() || {};
                rolActual = normalizarRol(perfilActual.rol);
                console.log("Acceso verificado para historial:", rolActual);
            }
        } catch (error) {
        ...
        fila.innerHTML = `
            ...
            <td>
                <div class="d-flex align-items-center gap-1 justify-content-center">
                    <input type="text" class="form-control form-control-sm text-center input-editable" 
                        ...
                        placeholder="Factura" ${(facturaValue !== '' || soloLectura()) ? 'disabled' : ''} style="width: 100px;">
                    
                    <button class="btn btn-sm p-0 border-0 btn-adjuntar-base64" data-id="${id}" title="${tieneFacturaAdjunta ? 'Ver Factura Adjunta' : 'Adjuntar Factura'}" ${(soloLectura() && !tieneFacturaAdjunta) ? 'disabled' : ''}>
            ...
            <td>
                <input type="text" class="form-control form-control-sm text-center input-editable" 
                    ...
                    placeholder="Código" ${(solpedCodeValue !== '' || soloLectura()) ? 'disabled' : ''}>
            </td>
            <td>
                <button class="btn btn-sm ${colorSolped} w-100 btn-estado" data-id="${id}" data-campo="estadoSolped" data-valor="${estadoSolped}" ${(estadoSolped !== 'PENDIENTE' || soloLectura()) ? 'disabled' : ''}>${estadoSolped}</button>
            </td>
            <td>
                <button class="btn btn-sm ${colorPago} w-100 btn-estado" data-id="${id}" data-campo="estadoPago" data-valor="${estadoPago}" ${(estadoPago !== 'PENDIENTE' || soloLectura()) ? 'disabled' : ''}>${estadoPago}</button>
            </td>
            ...
            <td>
                <div class="btn-group">
                    ...
                    ${soloLectura() ? '' : `<button class="btn btn-sm btn-outline-danger btn-anular" data-id="${id}"><i class="bi bi-x-circle"></i></button>`}
                </div>
            </td>
        `;
