...
const puedeCrearImportarProductos = () => modulosPorPerfil(perfilActual).includes('products');
const puedeEliminarProductos = () => ['admin','comprador'].includes(rolActual);
const puedeUsarEnRequerimiento = () => modulosPorPerfil(perfilActual).includes('rq_create');

onAuthStateChanged(auth, async (user) => {
    if (!user) return window.location.href = '../index.html';
    try {
        const snap = await getDoc(doc(db,'usuarios',user.uid));
        perfilActual = snap.exists() ? (snap.data() || {}) : {};
...
function actualizarBotonListo() {
    ...
    if (!puedeUsarEnRequerimiento()) { btn?.remove(); return; }
    ...
    productos.forEach(p => {
        ...
        tr.innerHTML=`
            ...
            <td class="text-center"><div class="btn-group">
                ${puedeUsarEnRequerimiento()?`<button class="btn btn-sm ${seleccionado?'btn-primary':'btn-success'} btn-usar" data-id="${p.id}"><i class="bi ${seleccionado?'bi-check-circle-fill':'bi-plus-circle-fill'}"></i> ${seleccionado?'AÑADIDO':'USAR'}</button>`:''}
...
function esc(v='') { return String(v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c])); }
 ...
 tablaProductos?.addEventListener('click', async e => {
    ...
    if(btn.classList.contains('btn-usar') && puedeUsarEnRequerimiento()) {
