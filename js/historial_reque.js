...
import { modulosPorPerfil } from './access-control.js';
...
let idActual = "";
let perfilActual = {};
const puedeGestionarRQ = () => modulosPorPerfil(perfilActual).includes('rq_create');
    ...
    if (user) {
        try {
            ...
            if (userDoc.exists()) {
                ...
                console.log("Acceso verificado para historial RQ");
            }
        } catch (error) {
        ...
        fila.innerHTML = `
            ...
            <td>
                <div class="btn-group">
                    ...
                    ${puedeGestionarRQ()?`<button class="btn btn-sm btn-outline-danger btn-anular-req" data-id="${id}"><i class="bi bi-x-circle"></i> Anular</button>`:''}
                </div>
            </td>
        `;
...
function renderModal(items, estadoGeneral) {
    ...
    items.forEach((item, index) => {
        ...
        const disabledOC = (ocValue !== "" || esAnulado || !puedeGestionarRQ()) ? "disabled" : "";
        const disabledSelect = (esAnulado || !puedeGestionarRQ()) ? "disabled" : "";
...
cuerpoModal?.addEventListener('change', async (e) => {
    if (!idActual || !puedeGestionarRQ()) return;
    const snap = await getDoc(doc(db, "requerimientos", idActual));
