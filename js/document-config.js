...
import { doc, getDoc, setDoc, serverTimestamp } from "./supabase-db-compat.js";

// Valores de respaldo. La pantalla administrativa puede modificarlos sin tocar código.
export const DEFAULT_CONFIG_DOCUMENTOS = {
    configVersion: 2,
    empresa: {
    ...
    compradores: [
        { nombre: "Paul Aster", tlf: "957 254 498", correo: "Paster@grupoqata.pe" },
        { nombre: "Dave Cardenas", tlf: "942 628 722", correo: "Dcardenas@grupoqata.pe" },
        { nombre: "Manuel Vega", tlf: "", correo: "" }
    ],
    proyectos: [
        { nombre: "CAMBRIDGE", direccion: "Colegio Cambridge, Chorrillos Alameda De Los Molinos 728-730, Chorrillos" },
        { nombre: "TORRE PRIMA", direccion: "Calle Chinchon 980, San Isidro" },
        { nombre: "GRID 154", direccion: "Calle Mario Valdivia 154, San Miguel" },
        { nombre: "WYNK", direccion: "Jr.Ucello 111, San Borja" },
        { nombre: "PISO 19A", direccion: "Av. EL Derby 2550, Santiago de Surco" },
        { nombre: "PISO 19B", direccion: "Av. EL Derby 2550, Santiago de Surco" },
        { nombre: "POSVENTA", direccion: "Av. Caminon Real 1236, San Isidro" },
        { nombre: "QUALITY", direccion: "Centro Comercial Puruchuco" },
        { nombre: "CORIL", direccion: "Av. EL Derby 2550, Santiago de Surco" },
        { nombre: "CARSA SJL", direccion: "" },
        { nombre: "ADMIN", direccion: "Av. Caminon Real 1236, San Isidro" },
        { nombre: "FUXION", direccion: "Av. EL Derby 2550, Santiago de Surco" }
    ],
    formasPago: ["CRÉDITO", "TRANSFERENCIA", "CAJA CHICA", "TARJETA", "ER"],
...
function normalizarProyectos(proyectos) {
    const base = Array.isArray(proyectos) && proyectos.length
        ? proyectos.map(p => ({ ...p }))
        : DEFAULT_CONFIG_DOCUMENTOS.proyectos.map(p => ({ ...p }));
    const cambridge = DEFAULT_CONFIG_DOCUMENTOS.proyectos.find(p => normalizarNombre(p.nombre) === 'CAMBRIDGE');
    const idx = base.findIndex(p => normalizarNombre(p?.nombre) === 'CAMBRIDGE');
    if (idx < 0 && cambridge) base.unshift({ ...cambridge });
    else if (idx >= 0 && !String(base[idx]?.direccion || '').trim() && cambridge) base[idx].direccion = cambridge.direccion;
    return base;
}

function normalizarCompradores(compradores) {
    const base = Array.isArray(compradores) && compradores.length
        ? compradores.map(c => ({ ...c }))
        : DEFAULT_CONFIG_DOCUMENTOS.compradores.map(c => ({ ...c }));

    // Migra datos antiguos sin volver a mostrar el usuario/comprador eliminado.
    let lista = base.filter(c => normalizarNombre(c?.nombre) !== 'JOHPSER ALEJANDRO');

    // Una versión antigua del ZIP traía Joseph Sandoval en lugar de Manuel Vega.
    // Solo se migra esa entrada heredada; cualquier otro comprador personalizado se conserva.
    const tieneManuel = lista.some(c => normalizarNombre(c?.nombre) === 'MANUEL VEGA');
    if (!tieneManuel) {
        const legado = lista.findIndex(c => normalizarNombre(c?.nombre) === 'JOSEPH SANDOVAL');
        if (legado >= 0) lista[legado] = { nombre: 'Manuel Vega', tlf: '', correo: '' };
        else lista.push({ nombre: 'Manuel Vega', tlf: '', correo: '' });
    }
...
function mezclarConfiguracion(data = {}) {
    const esConfiguracionAnterior = Number(data.configVersion || 0) < DEFAULT_CONFIG_DOCUMENTOS.configVersion;
    return {
        ...DEFAULT_CONFIG_DOCUMENTOS,
        ...data,
        configVersion: DEFAULT_CONFIG_DOCUMENTOS.configVersion,
        empresa: { ...DEFAULT_CONFIG_DOCUMENTOS.empresa, ...(data.empresa || {}) },
        compradores: normalizarCompradores(data.compradores),
        proyectos: normalizarProyectos(data.proyectos),
        formasPago: Array.isArray(data.formasPago) && data.formasPago.length ? data.formasPago : DEFAULT_CONFIG_DOCUMENTOS.formasPago,
        os: { ...DEFAULT_CONFIG_DOCUMENTOS.os, ...(data.os || {}) },
        // Las condiciones nuevas solicitadas sustituyen automáticamente las de versiones anteriores.
        terminosCondiciones: esConfiguracionAnterior
            ? DEFAULT_CONFIG_DOCUMENTOS.terminosCondiciones
            : (data.terminosCondiciones || DEFAULT_CONFIG_DOCUMENTOS.terminosCondiciones)
    };
}

export async function cargarConfiguracionDocumentos() {
    try {
        const snap = await getDoc(doc(db, "configuracion", "documentos"));
        return snap.exists() ? mezclarConfiguracion(snap.data()) : mezclarConfiguracion();
    } catch (error) {
        console.warn("No se pudo cargar configuración de documentos; se usarán valores de respaldo.", error);
        return mezclarConfiguracion();
    }
}

export async function guardarConfiguracionDocumentos(config) {
    const data = mezclarConfiguracion(config);
    await setDoc(doc(db, "configuracion", "documentos"), {
        ...data,
