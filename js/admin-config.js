...
async function cargarUsuariosRoles() {
    ...
    try {
        ...
        tbody.innerHTML = rows.length ? rows.map(r => {
            ...
            const label = ({admin:'Administrador',comprador:'Comprador',admin_obra:'Administrador de Obra',editor:'Administrador de Obra'})[rol] || rol || 'Sin rol';
            const fecha = r.updated_at ? new Date(r.updated_at).toLocaleString('es-PE') : '';
            return `<tr><td>${String(r.data?.email || '')}</td><td><span class="badge text-bg-light border">${label}</span></td><td class="small text-muted">${r.id}</td><td class="small">${fecha}</td></tr>`;
        }).join('') : '<tr><td colspan="4" class="text-center text-muted py-3">No hay perfiles ERP asignados.</td></tr>';
    } catch (error) {
        console.error(error);
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-danger py-3">No se pudo cargar la lista de usuarios.</td></tr>';
    }
}

async function asignarRolUsuario() {
    const email = $('usuarioRolEmail')?.value.trim().toLowerCase();
    const rol = $('usuarioRolSelect')?.value;
    if (!email) return mensaje('Ingresa el correo del usuario que ya creaste en Authentication → Users.', 'warning');
    const btn = $('btnAsignarRol');
    btn.disabled = true;
    try {
        const { data, error } = await supabase.rpc('qata_set_user_role', { p_email: email, p_role: rol });
        if (error) throw error;
        ...
        mensaje(msg.includes('USUARIO_AUTH_NO_ENCONTRADO') ? 'Ese correo todavía no existe en Authentication → Users. Créalo allí primero.' : `No se pudo asignar el rol: ${msg}`, 'danger');
    } finally {
        btn.disabled = false;
    }
}
...
$('btnAsignarRol')?.addEventListener('click', asignarRolUsuario);
