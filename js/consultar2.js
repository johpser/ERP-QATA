...
let perfilActual = {};
const puedeGestionarGuias = () => modulosPorPerfil(perfilActual).includes('guide_create');
    ...
    if (user) {
        try {
            ...
            if (userDoc.exists()) {
                ...
                console.log("Acceso verificado para historial de guías");
            }
        } catch (error) {
        ...
        fila.innerHTML = `
            ...
            <td>
                <div class="btn-group align-items-center gap-1">
                    ...
                    ${puedeGestionarGuias()?`<button class="btn btn-sm btn-outline-danger btn-anular" data-id="${id}" ${esAnulada ? 'disabled' : ''} title="Anular Guía"><i class="bi bi-trash"></i></button>`:''}
                </div>
            </td>
        `;
...
document.addEventListener('click', async (e) => {
    ...
    if (!btn || !puedeGestionarGuias()) return;
